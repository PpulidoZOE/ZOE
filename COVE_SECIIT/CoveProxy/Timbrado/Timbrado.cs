...
namespace CoveProxy.Timbrado
{
    public class Timbrador
    {
        ...
        /*
        internal string TimbrarPACSW(string archivoXML, string archivoXslt, string token, string urlTimbrado, string usuario, string password)
        {
            string cfdiReturn = string.Empty;
            try
            {
                //Autentificacion
                Authentication auth = new Authentication(urlTimbrado, usuario, password);
                AuthResponse response = auth.GetToken();
                token = response.data.token;

                //Timbrar Xml
                Stamp stamp = new Stamp(urlTimbrado, token);
                string xml = Encoding.UTF8.GetString(File.ReadAllBytes(archivoXML));
                StampResponseV3 timbrar = stamp.TimbrarV3(xml);
            }
            catch (Exception ex)
            {
      ...
      /* internal string TimbrarPACSW(string archivoXML, string archivoXslt, string archivoCertificado, string archivoKey, string certificadoPass, string userId, string userPass, string urlTimbrado)
        {
            string cfdiReturn = string.Empty;
            string layout = string.Empty;
            BasicHttpBinding binding = new BasicHttpBinding();
            ...
            try
            {
                ...
                {
                    layout = util.AgregarSello(archivoXML, archivoXslt, archivoCertificado, archivoKey, certificadoPass, versionCFDI);
                    Authentication auth = new Authentication(urlTimbrado, userId, userPass);
                    AuthResponse response = auth.GetToken();
                    token = response.data.token;
                    Stamp stamp = new Stamp(urlTimbrado, token);
                    StampResponseV3 timbrar = stamp.TimbrarV3(layout);
                }else
                {
                    cfdiReturn = "ERROR: Otras versiones de CFDI diferentes a 3.3 no es soportada";
                }
            }
            catch (Exception ex)
            {
